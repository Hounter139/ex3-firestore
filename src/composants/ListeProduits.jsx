...
export default function ListeProduits(props) {
  ...
  useEffect(() => {
    async function getProduits() {
      ...
      const resultat = await bd.collection("ex3-produits").get();
      
      /******* Ex#3 - Étape G ********************************/ 
      // Modifier l'état des produits (initialisé ci-dessus avec useState) en utilisant le mutateur et le tableau tabProduits
      resultat.forEach(produit=>tabProduits.push({id : produit.id,...produit.data()}));
  ...
  return (
    <div className="ListeProduits">
      ...
      <ul>        
        ...
        */}
          {
            produits.map(produit=><Produit etatPanier ={props.etatPanier} {...produit}/>)
          }
      </ul>
    </div>
  );
